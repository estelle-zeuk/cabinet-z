<x-jet-action-section>
    <x-slot name="title">
        {{ __('profile.browser-session') }}
    </x-slot>

    <x-slot name="description">
        {{ __('profile.manage-account-on-other-browsers') }}
    </x-slot>

    <x-slot name="content">
        <div class="max-w-xl text-sm text-gray-600">
            {{ __('profile.logout-account-on-other-browsers-if-session-has-been-compromised') }}
        </div>
        ...
        <div class="flex items-center mt-5">
            <x-jet-button wire:click="confirmLogout" wire:loading.attr="disabled">
                {{ __('profile.logout-other-browser-sessions') }}
            </x-jet-button>
        ...
        <x-jet-dialog-modal wire:model="confirmingLogout">
            ...
            <x-slot name="content">
                {{ __('profile.confirm-password-to-logout') }}

                <div class="mt-4" x-data="{}" x-on:confirming-logout-other-browser-sessions.window="setTimeout(() => $refs.password.focus(), 250)">
                    <x-jet-input type="password" class="mt-1 block w-3/4" placeholder="{{__('auth.password')}}"
                                x-ref="password"
